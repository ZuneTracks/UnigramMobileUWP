...
namespace Unigram.ViewModels
{
    public partial class DialogViewModel
    {
        ...
        public async void StickerSendExecute(Sticker sticker, bool? schedule, bool? silent, string emoji = null)
        {
            ...
            var input =
#if MODERN_TDLIB
                new InputMessageSticker(new InputSticker(new InputFileId(sticker.StickerValue.Id), sticker.Thumbnail?.ToInput(), sticker.Width, sticker.Height), emoji ?? string.Empty);
#else
                new InputMessageSticker(new InputFileId(sticker.StickerValue.Id), sticker.Thumbnail?.ToInput(), sticker.Width, sticker.Height, emoji ?? string.Empty);
#endif
        ...
        public async Task<MessageSendOptions> PickMessageSendOptionsAsync(bool? schedule = null, bool? silent = null)
        {
            ...
            if (schedule == true || (_type == DialogType.ScheduledMessages && schedule == null))
            {
                ...
                {
                    return ModernTdlibCompatibility.CreateMessageSendOptions(false, false, new MessageSchedulingStateSendWhenOnline());
                }
                else
                {
            ...
            {
                return ModernTdlibCompatibility.CreateMessageSendOptions(silent ?? false, false, null);
            }
        }
        ...
        private async Task<BaseObject> SendMessageAsync(Chat chat, long replyToMessageId, InputMessageContent inputMessageContent, MessageSendOptions options)
        {
            ...
            var response = await ProtoService.SendAsync(ModernTdlibCompatibility.CreateSendMessage(chat.Id, _threadId, replyToMessageId, options, null, inputMessageContent));
